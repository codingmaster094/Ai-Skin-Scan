...
export default function ScanPage() {
  ...
  // Poll for results if sessionId present
  useEffect(() => {
    if (!sessionId) return
    ...
    const interval = setInterval(async () => {
      try {
        const res = await fetch(`/api/scan?id=${sessionId}`)
        if (res.status === 404) {
...
function generateUUID() {
  ...
  if (sessionId) {
    // Show result or loading on PC after scanning
    return (
      <main className="min-h-screen bg-white text-black p-4 max-w-md mx-auto text-left font-sans">
        <h1 className="text-xl font-bold mb-4">Scan Results</h1>
        {polling && <p>Waiting for scan results...</p>}
        ...
        {result && (
          <>
            ...
            <h2 className="text-lg font-semibold mb-3 mt-6">Recommendations for you</h2>
